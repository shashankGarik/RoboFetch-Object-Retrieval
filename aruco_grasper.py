        ...
        for i in range(-3, 3):
            ...
            for marker in self.markers:
                if marker.text == aruco_name:
                    height_offset = 0 #rospy.get_param('/aruco_marker_info/{}/height_offset'.format(marker.id), default=0)
                    depth_offset = 0 #rospy.get_param('/aruco_marker_info/{}/depth_offset'.format(marker.id), default=0)
                    

                    displacement = self.get_displacement(marker)
                    print(displacement)
                    print(self.joint_controller.joint_states.position[Joints.joint_lift.value])
